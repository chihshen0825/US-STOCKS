...
function _showRejectFailsModal(sym, title, wrStr, fails) {
  ...
  const list = items.map((raw, i) => {
    ...
    // v.63: 額外提供「✏ 立即調整」浮窗，不離開 modal 即可改設定
    const adjustBtn = f.key
      ? `<button type="button" class="rfm-adjust" data-key="${esc(f.key)}" title="跳到該設定控制項並 focus／高亮">⚙ 調整</button>
         <button type="button" class="rfm-inline" data-key="${esc(f.key)}" title="在此浮窗即時調整，不離開本視窗">✏ 立即調整</button>`
      : "";
    return `
      <li class="rfm-item">
  ...
  mask.innerHTML = `
    <div class="confirm-card rfm-card" role="dialog" aria-modal="true">
      ...
      <div class="confirm-body rfm-body">
        <div class="rfm-summary">共 <b>${items.length}</b> 項不符規則。<b>⚙ 調整</b>＝跳到設定面板；<b>✏ 立即調整</b>＝在此浮窗即時改值（不離開本視窗）。</div>
  ...
  // v.63: 點「✏ 立即調整」→ 不關 modal，於此處彈出小浮窗即時調整
  mask.querySelectorAll(".rfm-inline").forEach(btn => {
    btn.addEventListener("click", (ev) => {
      ev.preventDefault(); ev.stopPropagation();
      const k = btn.dataset.key;
      try { _inlineAdjustSetting(k, btn); } catch (err) { try { console.warn("[inlineAdjust]", err); } catch {} }
    });
  });
  setTimeout(() => { try { mask.querySelector(".btn-ok")?.focus(); } catch {} }, 0);
}
...
async function openUpdateStatusPopover() {
  ...
  const status = !repo
    ...
    : !enabled
      ? `<span style="color:#ff9800">⏸ 自動檢查已停用</span>`
      : !info
  ...
  pop.innerHTML = `
    ...
    <div class="ub-notes" style="max-height:none;">
      ...
      <div style="margin-top:10px;">
        <label style="display:flex;align-items:center;gap:6px;color:#e6e6e6;cursor:pointer;user-select:none;">
          <input id="updEnabledInput" type="checkbox" ${enabled ? "checked" : ""} style="margin:0;cursor:pointer;">
          <span>啟用自動檢查更新（每 6 小時一次）</span>
        </label>
        <div style="color:#8a93a0;font-size:11px;margin-top:4px;margin-left:22px;">停用後不會向 GitHub 發出請求；仍可手動按「立即檢查」。</div>
      </div>
      <div style="margin-top:10px;">
  ...
  pop.querySelector("[data-act=save]")?.addEventListener("click", () => {
    ...
    const en = !!pop.querySelector("#updEnabledInput")?.checked;
    chrome.storage.local.set({ dash_update_repo: v, dash_update_enabled: en, [UPDATE_DISMISS_KEY]: null }, () => {
      if (en) {
        ...
        // 停用：不發 API 請求，直接重開彈窗
        pop.remove();
        setTimeout(openUpdateStatusPopover, 200);
      }
    });
  });
  pop.querySelector("[data-act=check]")?.addEventListener("click", () => {
    // 手動檢查不受啟用狀態限制（送一次性 message）
    chrome.runtime.sendMessage({ type: "checkUpdateNow", force: true }, () => {
      pop.remove();
      setTimeout(openUpdateStatusPopover, 600);
    });
  });
